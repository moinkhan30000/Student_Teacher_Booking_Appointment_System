...
function sameDayISO(d: Date) {
  return d.toISOString().slice(0,10);
}

// Merge & sort busy blocks for one day
function addBusyDay(acc: Busy[], date: string, ranges: Range[], source: string) {
  for (const r of ranges) {
    if (r.end > r.start) acc.push({ date, start: r.start, end: r.end, source });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Load admin settings + teacher availability + approved appts
    const [adminDoc, availDoc, apptsSnap] = await Promise.all([
    ...
    const weekly: Record<string, Range[]> = avail?.weekly || {}; // teacher class hours
    ...
    // Build day-by-day busy
    const out: Busy[] = [];

    // Admin holidays → full-day busy (00:00-23:59)
    ...
    // For each day in range: add teacher weekly class hours (recurring busy)
    for (let t = fromD.getTime(); t <= toD.getTime(); t += 86400000) {
      ...
      // clip to admin work window
      const clipped = wr
        .map(r => ({ start: r.start >= workStart ? r.start : workStart, end: r.end <= workEnd ? r.end : workEnd }))
        .filter(r => r.end > r.start);
      addBusyDay(out, dateISO, clipped, "teacher-weekly");
    }

    // Ad-hoc busy already collected above (busyList)
    out.push(...busyList.filter(b => b.date >= from && b.date <= to));

    // Approved appointments → busy
    apptsSnap.forEach(doc => {
      ...
      out.push({ date: d, start, end, source: "appointment" });
    });

    // Also return admin work window so the client can limit pickers
